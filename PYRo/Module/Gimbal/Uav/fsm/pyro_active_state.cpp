...
void uav_gimbal_t::state_active_t::execute(uav_gimbal_t *owner)
{
    ...
    // owner->gimbal_ctx.data.yaw_angle_increase = owner->gimbal_ctx.data._target_yaw_angle - (- owner->gimbal_ctx.data._current_yaw_angle);
    //
    // //yaw轴限位
    // if ((owner->gimbal_ctx.data.yaw_motor_angle + owner->gimbal_ctx.data.yaw_angle_increase) < yaw_max_value
    //     && (owner->gimbal_ctx.data.yaw_motor_angle + owner->gimbal_ctx.data.yaw_angle_increase) > 0)
    // {
    //     owner->gimbal_ctx.data.real_target_yaw_angle = yaw_max_value;
    // }
    // else if ((owner->gimbal_ctx.data.yaw_motor_angle + owner->gimbal_ctx.data.yaw_angle_increase) > yaw_min_value
    //     && (owner->gimbal_ctx.data.yaw_motor_angle + owner->gimbal_ctx.data.yaw_angle_increase) < 0)
    // {
    //     owner->gimbal_ctx.data.real_target_yaw_angle = yaw_min_value;
    // }
    // else owner->gimbal_ctx.data.real_target_yaw_angle = owner->gimbal_ctx.data.yaw_motor_angle + owner->gimbal_ctx.data.yaw_angle_increase;
    //
    // normalize_angle(owner->gimbal_ctx.data._target_yaw_angle);
    ...
    // roll目标值 归一化到-pi到pi之间
     ...
     if (owner->gimbal_ctx.data.pitch_motor_angle < 3.04f && owner->gimbal_ctx.data.pitch_motor_angle > 2.3f)
     {
     ...
     // owner->gimbal_ctx.data.roll_angle_increase = owner->gimbal_ctx.data._target_roll_angle + owner->gimbal_ctx.data._current_roll_angle;
     //
     // if (owner->gimbal_ctx.data.roll_motor_angle + owner->gimbal_ctx.data.roll_angle_increase > roll_max_value)
     // {
     //     owner->gimbal_ctx.data.real_target_roll_angle = roll_max_value;
     // }
     // else if (owner->gimbal_ctx.data.roll_motor_angle + owner->gimbal_ctx.data.roll_angle_increase < roll_min_value)
     // {
     //     owner->gimbal_ctx.data.real_target_roll_angle = roll_min_value;
     // }
     // else    owner->gimbal_ctx.data.real_target_roll_angle = owner->gimbal_ctx.data.roll_angle_increase
     //                                                     + owner->gimbal_ctx.data.roll_motor_angle;
     //
     // normalize_angle(owner->gimbal_ctx.data.real_target_roll_angle);
