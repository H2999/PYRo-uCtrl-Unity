...
void uav_gimbal_t::state_active_t::execute(uav_gimbal_t *owner)
{
    ...
    if (owner->gimbal_ctx.data.yaw_motor_angle > 0)
    {
        owner->gimbal_ctx.data.yaw_real_max_limit_angle = owner->gimbal_ctx.data.yaw_motor_angle - yaw_max_value
                                            - owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle;
        owner->gimbal_ctx.data.yaw_real_min_limit_angle = owner->gimbal_ctx.data.yaw_motor_angle - (2 * PI + yaw_min_value)
                                            - owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle;
    }
    if (owner->gimbal_ctx.data.yaw_motor_angle < 0)
    {
        owner->gimbal_ctx.data.yaw_real_max_limit_angle = (2 * PI + owner->gimbal_ctx.data.yaw_motor_angle) - yaw_max_value
                                            - owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle;
        owner->gimbal_ctx.data.yaw_real_min_limit_angle = owner->gimbal_ctx.data.yaw_motor_angle - yaw_min_value
                                            - owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle;
    }

    if (owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle > owner->gimbal_ctx.data.yaw_real_max_limit_angle)
    {
        owner->gimbal_ctx.data._target_yaw_angle = owner->gimbal_ctx.data.yaw_real_max_limit_angle;
    }
    if (owner->gimbal_ctx.data.correct_imu_ctx.correct_yaw_angle < owner->gimbal_ctx.data.yaw_real_min_limit_angle)
    {
